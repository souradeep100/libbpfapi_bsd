...
static int create_map_freebsd(uint32_t map_type, uint32_t key_size, uint32_t value_size, uint32_t max_entries,
...
const EbpfProgramType freebsd_socket_filter_program_type =
    PTYPE("socket_filter", &g_socket_filter_descr, BPF_PROG_TYPE_SOCKET_FILTER, {"socket"});

const EbpfProgramType freebsd_xdp_program_type =
...
const std::vector<EbpfProgramType> freebsd_program_types = {
    PTYPE("unspec", &g_unspec_descr, BPF_PROG_TYPE_UNSPEC, {}),
    freebsd_socket_filter_program_type,
    freebsd_xdp_program_type,
    ...
    // type but should be mapped to the relevant native freebsd program type
    // value.
...
static EbpfProgramType get_program_type_freebsd(const std::string& section, const std::string& path) {
    EbpfProgramType type{};

    // freebsd only deduces from section, but cilium and cilium_test have this information
    // in the filename:
    ...
    if (path.find("cilium") != std::string::npos) {
        if (path.find("xdp") != std::string::npos) {
            return freebsd_xdp_program_type;
        }
    ...
    for (const EbpfProgramType& t : freebsd_program_types) {
    ...
    return freebsd_socket_filter_program_type;
}

#define BPF_MAP_TYPE(x) 0, #x

static const EbpfMapType freebsd_map_types[] = {
    {BPF_MAP_TYPE(UNSPEC)},
    {BPF_MAP_TYPE(HASH)},
...
EbpfMapType get_map_type_freebsd(uint32_t platform_specific_type)
{
    uint32_t index = platform_specific_type;
    if ((index == 0) || (index >= sizeof(freebsd_map_types) / sizeof(freebsd_map_types[0]))) {
        return freebsd_map_types[0];
    }
    EbpfMapType type = freebsd_map_types[index];
    type.platform_specific_type = platform_specific_type;

    return type;
}

void parse_maps_section_freebsd(std::vector<EbpfMapDescriptor>& map_descriptors, const char* data, size_t map_def_size, int map_count, const ebpf_platform_t* platform, ebpf_verifier_options_t options)
{
    ...
    for (auto const& s : mapdefs) {
        EbpfMapType type = get_map_type_freebsd(s.type);
        map_descriptors.emplace_back(EbpfMapDescriptor{
            .original_fd = create_map_freebsd(s.type, s.key_size, s.value_size, s.max_entries, options),
            .type = s.type,
            .key_size = s.key_size,
                            ...
                            ebpf_verifier_options_t options)
{
    if (options.mock_map_fds) {
        EbpfMapType type = get_map_type_freebsd(map_type);
        return create_map_crab(type, key_size, value_size, max_entries, options);
    }

    throw std::runtime_error(std::string("cannot create a freebsd map"));
}

EbpfMapDescriptor& get_map_descriptor_freebsd(int map_fd)
{
...
const ebpf_platform_t g_ebpf_platform_freebsd = {
    get_program_type_freebsd,
    get_helper_prototype_freebsd,
    is_helper_usable_freebsd,
    sizeof(bpf_load_map_def),
    parse_maps_section_freebsd,
    get_map_descriptor_freebsd,
    get_map_type_freebsd,
};
